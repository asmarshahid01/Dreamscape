  ...
  return (
    <div className='public-dreams-container'>
        {publicDreams.map((publicDream, index) => {
            return <div className={commentsExpanded && index === publicDreamSelected ? 'public-dream-expanded' : 'public-dream'} key={index}>
                <div className={commentsExpanded && index === publicDreamSelected ? 'expand-comments-btn-inverted' : 'expand-comments-btn'} onClick={() => {setCommentsExpanded(true); setpublicDreamSelected(index)}}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M12 15.0006L7.75732 10.758L9.17154 9.34375L12 12.1722L14.8284 9.34375L16.2426 10.758L12 15.0006Z"></path>
                    </svg>
                </div>
                <div className='public-dream-title-container'>
                    <p className={commentsExpanded && index === publicDreamSelected ? 'public-dream-title-inverted' : 'public-dream-title'}>{publicDream.title}</p>
                    <p className={commentsExpanded && index === publicDreamSelected ? 'public-dream-date-inverted' : 'public-dream-date'}>{publicDream.date}</p>
                </div>
                <p className={commentsExpanded && index === publicDreamSelected ? 'public-dream-txt-inverted' : 'public-dream-txt'}>{publicDream.dream}</p>
                {commentsExpanded && index === publicDreamSelected && publicDream.comments.map((publicComment, NestedIndex) => {
                    return <div className='public-dream-comment' key={NestedIndex}>
                        <div className='comment-container'>
                            <p className='commenter-name'>{publicComment.user}:</p>
                ...
                <div className='public-dream-comments'>
                    ...
                    className={commentsExpanded && index === publicDreamSelected ? 'public-dream-comment-input-inverted' : 'public-dream-comment-input'}
                    ...
                    <div className={commentsExpanded && index === publicDreamSelected ? 'post-user-comment-inverted' : 'post-user-comment'}>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
